...
DevOps vs Agile: What's the difference? DevOps和敏捷: 究竟有什么区别?
======

The difference between the two is what happens after development. 两者之间的区别在于开发完毕之后发生的事情。
![Pair programming][1]
...
早期, 软件开发并没有特定的管理流程。随后出现了<u>瀑布开发流程</u>, 它提出软件开发活动可以被开发和构建应用所耗费的时间来定义。
...
瀑布流程与<u>三重约束模型</u>相关, 三重约束模型也称为项目管理三角形。三角形的每一个边代表项目管理三要素的一个要素: **范围, 时间和成本。**正如<u>Angelo Baretta写到</u>, 三重约束模型认为成本是时间和范围的函数, 这三个约束以一种确定的, 可预测的方式相互作用。如果我们想缩短时间, 就必须增加成本。如果我们想增加范围, 就必须增加成本或时间。

### Transitioning from waterfall to agile 从瀑布流程过渡到敏捷开发
...
这与瀑布流程死板的计划风格相反。在敏捷流程中，客户是开发团队的一员，而不仅仅是在项目开始时参与项目需求的定义, 在项目结束时验收最终的产品。客户帮忙团队完成<u>准入标准</u>，保持参与整个流程。另外，敏捷需要整个组织的变化和持续的改进。开发团队和其他团队一起合作, 包括项目管理团队和测试团队。做什么和计划什么时候做由指定的角色领导, 并由整个团队同意。
...
所有这些已经被单独用于或一起用于开发和部署软件。最常用的是<u>scrum</u>, kanban(或scrumban)和DevOps.
...
<u>Scrum</u>是一个框架, 采用该框架的团队通常由一个scrum教练, 产品经理和开发人员组成, 该功能团队采用自主的工作方式, 能够加快软件交付速度从而给客户带来巨大的商业价值。其关注点是<u>较小增量</u>的快速迭代。
...
<u>Kanban</u>是一个敏捷框架，有时也叫工作流管理系统，它能帮助团队可视化他们的工作从而最大化效率。Kanban通常由数字或物理展示板来呈现。团队的工作移到展示板上，例如未启动，进行中, 测试中, 已完成。Kanban使得每个团队成员可以随时看到所有工作的状态。
...
DevOps基于两个其他实践: 精益和敏捷。DevOps不是一个公司内的岗位或角色；它是一个组织或团队对持续交付，持续部署和持续集成的坚持不懈的追求。<u>Gene Kim</u>(Phoenix项目和Unicorn项目的作者)认为，有三种方式定义DevOps的理念：

  * The First Way: Principles of flow 第一种: 自动化流程
  * The Second Way: Principles of feedback 第二种: 快速反馈
  * The Third Way: Principles of continuous learning 第三种: 持续学习



### DevOps software development DevOps软件开发
...
当你想到自动化，云，微服务时，你会想到DevOps。在一次<u>访谈</u>中, "*加速构建和扩张高性能技术组织"*的作者Nicol Forsgren, Jez Humble和Gene Kim这样解释到：
...
#### Differences 不同之处
  ...
  * **The difference between the two** is what happens after development. 两者的差异在于软件开发完成后发生的事情。
...
Agile 敏捷 | DevOps
---|---
Feedback from customer 从客户得到反馈 | Feedback from self 从自己得到反馈 
Smaller release cycles 较小的发布周期 | Smaller release cycles, immediate feedback 较小的发布周期，立即反馈 
Focus on speed 聚焦于速度 | Focus on speed and automation 聚焦于速度和自动化 
Not the best for business 对业务不是最好 | Best for business 对业务最好 
