...
在本文中，我们将向您介绍在Linux机器中添加新磁盘的步骤。将原始磁盘添加到 Linux 机器可能非常依赖于您所拥有的服务器类型，但是一旦将磁盘提供给机器，将其添加到挂载点的过程几乎相同。 

**目标** : 向服务器添加新的10GB磁盘，并使用lvm和新创建的卷组创建5GB装载点。

* * *

### 向Linux机器添加原始磁盘

如果您使用的是 AWS EC2 Linux 服务器，您可以 [按照以下步骤][2] 添加原始磁。 如果您使用的是 VMware Linux VM，那么您将有不同的步骤来添加磁盘。如果您正在运行物理机架安装/刀片服务器，那么添加磁盘将是一项物理任务。

现在，一旦磁盘物理地/虚拟地连接到 Linux 机器上，它将被内核识别，然后我们就开始了。

* * *

### 识别 Linux 最新添加的磁盘

原始磁盘连接后，您需要让内核去 [扫描新磁盘][3]。在新版中，它现在主要是由内核自动完成。

第一件事是在内核中识别新添加的磁盘及其名称。实现这一点的方法有很多。我将列举少量 –

  * 可以在添加/扫描磁盘之前和之后观察 `lsblk` 输出，以获取新的磁盘名。
  * 检查 `/dev` 文件系统中新创建的磁盘文件。匹配文件和磁盘添加时间的时间戳。
  * 观察 `fdisk-l` 添加/扫描磁盘前后的输出，以获取新的磁盘名。



对于我们的示例，我使用的是 AWS EC2 服务器，并向服务器添加了 5GB 磁盘。这是我的 lsblk 输出 –
...
您可以看到 xvdf 是我们新添加的磁盘。磁盘的完整路径是 `/dev/xvdf`。

* * *

### 在 LVM 中添加新磁盘

我们这里使用 LVM，因为它是在 Linux 平台上广泛使用的非常灵活的卷管理器。确认您已安装 `lvm` 或 `lvm2` [安装在系统上的程序包][4]. 如未安装， [安装 lvm/lvm2 程序包][5].
...
如果要将磁盘添加到现有挂接点，并使用其空间[扩展挂接点][9] ，则 `vgcreate` 应替换为 `vgextend`。
...
现在, 已完成逻辑卷创建。您需要使用所选的文件系统格式化它，并将其装入。我们在这里选择ext4文件系统，并使用 `mkfs.ext4` 进行格式化。
...
让我们使用 `mount` 命令在 /data 安装点上安装我们创建并格式化的5GB逻辑卷。
...
使用df命令验证您的挂载点，如上所述，您都完成了！您可以始终在 [/etc/fstab][10] 中添加一个条目，以便在重新启动时保持此装载。
