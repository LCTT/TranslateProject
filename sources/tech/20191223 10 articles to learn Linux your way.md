...
2019年，术语"Linux"有了更多含义：内核? 桌面? 或是生态？在此次回顾年度Linux好文中，我在选择十大好文时有意采取了更开放的视角。下面就是十大好文(无先后次序之分)。
...
Gaurav Kamathe的[_通过strace_来理解Linux的系统调用][9]具有很强的技术性，我希望我所见过的有关**strace **的每次会议演讲都是如此。 这是对一个复杂但非常有用的命令的清晰演示。令我惊讶的是，我读了本文才发现自己一直使用的命令不是系统调用命令，而是**ltrace**（以查看命令调用了哪些函数）。本文包含了丰富的信息，是开发人员和测试人员的快捷参考手册。
