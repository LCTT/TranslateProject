...
Ubuntu 20.10 (Groovy Gorilla) 今天停止支持啦(2021年7月22日)。它是一个非长期支持发行版本(non-LTS)[带来了一些令人兴奋的新特性][1].

通常，非长期支持发行版维护9个月，所以20.10生命周期结束意味着Ubuntu 20.10用户将没有安全和维护更新。

你也会错过已安装应用的更新，[使用apt命令][2]安装新软件也会面临问题。如果不手动个性sources.list(不推荐)，使用软件中心也将是个问题。
