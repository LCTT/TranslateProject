...
另外。感谢Liam Dawe(通过[GamingOnLinux][5])，和各自的开发者协调，成功在Steam Deck上测试了英雄游戏启动器 (和[英雄Bash启动器][6])。

![][7]

如果你好奇：**英雄Bash启动器**_是一个为所有已安装的英雄游戏创建启动脚本(.sh文件)的工具，允许你直接从终端或者游戏前端/启动器启动游戏，而不心打开英雄。_

这里是这一切是如何发生的(根据我与英雄Bash启动器开发者的简短交谈):

最初，在Steam Deck上进行Epic游戏商店实验时，Steam控制器无法工作，因为Epic游戏商店作为一个“非Steam游戏”，使用Steam客户端运行。

所以，英雄Bash启动器的开发者, [Rishabh Moharir][8] (也是这里的一位作者)建议使用他的工具，根据他的[GitHub上的wiki指南][9]来使其工作.
...
接下来，你需要下载最新的[legendary][10]的二进制文件，并将其设置为启动器设置中的可选传奇二进制文件。

你需要在启动器的游戏设置中配置和设置兼容层为Proton 7.0。

这时，你需要下载最新的[英雄Bash启动器二进制文件][11]，然后运行它。
...
对于我，这听起来可行，应该不会超越大多数 用户的能力。不幸的是，我不能进入到到印度的Steam Deck。
