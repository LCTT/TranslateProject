...
Free Software Foundation’s founding President [Richard Stallman was forced to resign][1] after a media outrage in 2019. The iconic hippie techie made a surprise [comeback announcement][2] during LibrePlanet event in March.
...
The episode divided the open source world once again. Many organizations and developers opposed his return. At the same time, Stallman also got [overwhelming support from non-USA developers][3].

Though [Stallman apologized][4], many people from GNOME and Open Source Initiative [advocated to remove entire FSF board.][5]
...
This [archinstall script is available in the newer Arch Linux ISO][7] can can be accessed by typing archinstall in the TTY while installing Arch Linux.

### 3\. Microsoft Edge browser comes to Linux

![][8]

It was already in beta for over a year, but 2021 sees the [final stable release of Microsoft’s Chromium-based Edge browser for Linux][9].
...
Several users who [installed Edge browser on Linux][10] found it surprisingly good. Personally, I still prefer Firefox over any other browser but if Edge could bring Ultra HD or Full HD Netflix streaming to Linux, it could become my secondary browser.

### 4\. Rise of CentOS replacements

Red Hat shocked the Linux world last years when it decided to [kill the stable CentOS in favor of the rolling release version CentOS Stream][11].

This gave birth to [several CentOS replacement distributions][12]. Two distros that got the most of the popularity and CentOS’ market share are Rocky Linux and Alma Linux.

[Rocky Linux is created by Gregory M. Kurtzer][13], the original creator of CentOS.

[Alma Linux][14] is created by CloudLinux, an enterprise which was already providing CentOS based servers and kernel patching service.

These two distros have captured the market left open by CentOS. [Red Hat’s plan of giving free RHEL licenses to small businesses][15] was not as successful.
...
Second, Steam claimed that all games (whether they are Windows-only) will run on Steam Deck. That’s too big a claim but [Steam has started verifying games that work with Deck][16]. This could mean the Deck verified games should work well on desktop Linux as well.

![][17]

### 6\. Anti-Cheat Engines officially supporting Linux

Another big news for Linux gamers in 2021. [Epic Games added complete Linux support for its Easy-Anti Cheat service, along with official SteamPlay (or Proton) and W][18]ine compatibility.
...
[GNOME 40 was a radical release][20]. Almost as radical as GNOME 3.
...
Linus Sebastian had a difficult time with Linux. He struggled at most points and [ended up destroying his Pop!_OS install while trying to install Steam.][22]
...
Apple’s ARM based M1 series is a huge hit among the users for its performance boost. There is a dedicated project [Asahi][24] that is working to make Linux work on the Apple M1 devices.

The year 2021 saw some good progress by Asahi Linux. Their work is going to benefit other distributions as well.

![Arch Linux with KDE Plasma running on a M1 Pro MacBook | Image Credit][25]
