...
当你编写一个应用时，需要数据存储是很常见的。有时你要存储你的应用需要的资产，其他时候你要存储用户数据，包括偏好和保存数据。存储数据的一种方式是在数据库中，为了在你的代码和数据库之间进行通信，你需要为你的语言提供一个数据库绑定或连接器。对于 Java 来说，一个常见的数据库连接器是 JDBC（Java 数据库连接）。

### 1. 安装 Java

当然，要使用 Java 进行开发，你还必须安装 Java。我推荐 [SDKman][2] 用于 Linux、macOS 和 WSL 或 Cygwin。对于 Windows，你可以从 [developers.redhat.com][3] 下载 OpenJDK。
...
你必须安装要通过 JDBC 连接的数据库。有几个非常好的开源数据库，但是我不得不为这篇文章选择一个，所以我选择了 PostgreSQL。
