...
Linux 的目录结构有很多有趣的特点。 This month I cover some fascinating aspects of the /dev directory. Before you proceed any further with this article, I suggest that, if you have not already done so, you read my earlier articles, _[Everything is a file][1]_, and _[An introduction to Linux filesystems][2]_, both of which introduce some interesting Linux filesystem concepts. Go ahead—I will wait.
Linux 的目录结构有很多有趣的特点。这个月我要讲一些 /dev 目录的迷人之处。在你读这篇文章之前，如果你还没开始的话，我建议你读一下我以前的几篇文章，_[一切皆文件][1]_ 和_[Linux 文件系统介绍][3]_，两篇都介绍了一些有趣的 Linux 文件系统的概念。去把，我等不急了。

Great! Welcome back. Now we can proceed with a more detailed exploration of the /dev directory.
很好！欢迎回来。现在我们可以对 /dev 目录进行更详细的探索了。
...
设备文件也叫[设备][3][特殊文件][4]。设备文件用于向操作系统和用户提供他们所代表的设备的接口。所有的 Linux 设备文件都在 /dev 目录里面，它是根目录(/)的组成部分，因为这些设备文件在系统启动的时候要能为系统所用。
...
对于设备文件，有个很重要的事要记清楚，它们绝对不是设备驱动程序。它们更准确的描述是设备驱动程序的入口。数据从应用程序或者操作系统传输到设备文件然后传输到设备驱动程序最后传输到物理设备。反向数据路径也用，从物理设备到设备驱动到设备文件再到应用程序或者另一个设备。
