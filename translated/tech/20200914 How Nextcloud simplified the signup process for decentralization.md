...
这就导致了一些问题：
  ...
  * 🔮 人们不知道该选哪个，有错失恐惧症（Fear Of Missing Out），然后根本不选。这就是选择的悖论！
...
所以，我们不断尝试为任意一个 Nextcloud 桌面和移动应用改进这一点，比如 Nextcloud Talk 或者所有优秀的社区开发的应用。在 Android 上，我们与 DAVx5（Android 上的日历和联系人同步）紧密集成，而且，对于其他 Android 应用，还有一个[单点登录库][13]。不幸的是，在 iOS 上，就没有那么容易了，因为应用必须来自同一个开发者才能共享凭证。
