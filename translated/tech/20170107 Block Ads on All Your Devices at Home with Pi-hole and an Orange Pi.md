...
你是否发现在你的浏览器、智能手机和平板上不得不设置广告拦截器很令人恼火？ 至少我是这样的。我家里有一些“智能”设备，然而它们似乎没有任何类型的广告拦截软件。 好了，我了解到 [Pi-hole][2] 是一个 pi 上的广告拦截软件，它能在拦截你的家庭网络上的所有广告，在它们到达你的各种设备之前。它能让你列出任何域的黑名单或白名单，并且它有一个很好的面板，让你深入了解你的家庭网络最常访问的域/网站、最活跃的设备和最常见的广告商。
...
我没有选择 Pi-hole 主页上的“一键”安装。 我的意思是，他们在命令下面写着“管道到 bash 可能是危险的”，本地安装是“更安全”。所以，这里是我的本地安装步骤：
...
如果你没有以 root 用户身份登录，那么基本安装脚本将提示你输入密码，然后再继续。 如果需要，脚本将下载并安装一些必备的 Linux 软件包。接着它会显示一个红蓝界面，提示你回答有关如何配置 Pi-hole 的几个问题。以我的经验，直接接受默认值就可以了，我后来发现 Pi-hole 的 web 程序可以更改设置，如 DNS 提供商。
...
上面的说明与 Pi-hole 网站提供的替代“安全”方法大致相同，尽管 Armbian 没有被列为官方支持的操作系统。我相信这些说明应该在 Raspberry Pi 或其他运行某种形式的基于 Debian 的 Linux 操作系统的 pi 上工作。但是，我并没有测试这一点，所以期待听到你的经验（请给我留下简短的评论）。
