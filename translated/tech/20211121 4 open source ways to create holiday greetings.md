...
节日再次来临，今年我决定用[开源方式][2]来庆祝。就像一个特别有名的假期忙人一样，我有一个长长的节日任务清单（我打算检查两遍）：制作一张贺卡（带地址的信封）寄给家人和朋友，用合适的节日歌曲制作照片蒙太奇或视频，并装饰我的虚拟办公室。有很多开源的应用和资源使我的工作更容易。以下是我使用的东西。

### Inkscape 和剪贴画

我最喜欢的资源之一是 [FreeSVG.org][3]（前身是 Openclipart.org）。这里很容易找到你喜欢的节日，包括光明节、圣诞节、新年等等。这些剪贴画都是由像你和我这样的用户贡献的，并且是 CC0 署名，所以你甚至不需要提供署名。在可能的情况下，我还是会注明出处，以确保 FreeSVG 和它的艺术家们获得知名度。
...
使用 [Inkscape 的][6] Text to Path 工具，我在图片上添加了我自己的文字，我把它用在卡片上。如果再准备一下，我还可以把这个图形用在一些定制的杯子或餐垫上。
...
[LibreOffice][9] Writer 可以用来制作贺卡和海报，在你的家中使用或分发给你的朋友和家人。使用 LibreOffice Calc 创建一个关于你的家人和朋友的数据库，然后利用这一资源，用邮件合并功能简化制作邮件标签。

### Creative Commons 图片和图形

[search.creativecommons.org][10] 上也有艺术品。注意许可证类型：对任何需要归属的东西给予适当的信用。这张图片（[“Thanksgiving Dealies”][11]）来自知识共享图片搜索。它的作者是 [Martin Cathrae][12]，并在 [CC BY-SA 2.0][13] 下获得许可，因此它可以在相同的许可下被改编、重复使用和分享。
...
知识共享组织（Creative Commons）提供了大量的图片选择，在你的下一次[视频会议][18]期间，这些图片将成为一个节日背景。

### 视频和实时流媒体

你也可以把这些图片与你自己的一些图片结合起来，用 [OpenShot][19] 视频编辑器创建一个简短的视频剪辑。你可以通过使用 Audacity 录制一个单独的音轨，轻松地添加旁白。声音效果可以在 Audacity 中添加，保存到文件中，并导入到 OpenShot 视频编辑器的配乐中。寻找[合法的背景][20]音乐来添加到你的视频中。

使用[开源广播软件][21]直播你的假期聚会。 使用 OBS 可以轻松地为你的朋友和家人呈现引人入胜的假期节目，或者你可以将节目另存为 Matroska 或 MP4 文件以供日后观看。

### 阅读材料

古腾堡计划是一个很好的用来分享的免费节日阅读材料的来源。[狄更斯的《圣诞颂歌》][22]就是这样一种资源，它可以在网上轻松阅读，也可以下载成 EPUB 或你最喜欢的电子阅读器的格式。你还可以找到免版税的阅读材料，如 [Librivox][23] 的“光的盛宴”，采用 MP3 格式，因此可以下载并在你喜欢的浏览器或媒体播放器中播放。

### 节日乐趣

节日里最重要的一点是，它们是与朋友和家人一起的放松和欢乐时光。如果你有家人对计算机感到好奇，花点时间与他们分享一些你最喜欢的开源资源。

Anderson Silva 向我们展示了如何使用 Raspberry Pi 和 LightshowPi 来创造一个音乐灯光秀。
