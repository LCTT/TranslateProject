...
修复在 Ubuntu 和其他 Linux 发行版上 “Unable to acquire dpkg frontend lock. Are You Root?” 的错误
======
...
一个错误的 sudo 尝试被添加到系统日志中。它记录了用户名、虚拟终端号、运行命令的位置和运行的命令。
...
当你不能用当前的用户账户使用 sudo 时，你能做什么？首先，[验证该用户是否有 sudo 权限。如果没有，你可以有一些选择][7]：
  ...
  * [将该用户加入 sudoer 列表][8] （如果你有其他用户账户的管理/sudo 权限）。
...
你不会在桌面 Linux 上看到如此细化的 sudo 配置，在桌面 Linux 上，它被预先配置为允许 sudo 组中的任何用户以 sudo 的身份运行任何命令。关于 sudo 的更多信息，请参见其他文章。
