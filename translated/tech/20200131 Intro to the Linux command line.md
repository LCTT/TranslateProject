...
如果你是 Linux 新手，或者从来没有花时间研究过命令行，你可能不会理解为什么这么多 Linux 爱好者坐在舒适的桌面使用大量工具与应用时键入命令产生的兴奋。在这篇文章中，我们将快速浏览一下命令行的奇妙之处，看看能否让你着迷。
...
一旦你得到你的命令行窗口，你会发现自己坐在一个提示面前。它可能只是一个 **$** 或者像 “**user@system:~$**” 这样的东西，但它意味着系统已经准备好为你运行命令了。
...
ps     列出我正在运行的进程（可能只是你的shell和“ps”命令）                
```
