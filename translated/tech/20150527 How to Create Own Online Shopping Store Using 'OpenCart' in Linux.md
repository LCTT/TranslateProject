...
1. 像我说的，OpenCart需要一定的技术要求比如在系统上安装Apache，PHP扩展和数据库(MySQL或MariaDB)，为了运行Opencart正常。
...
10. 下一个屏幕是安装前检查服务器设置，查看服务器所需的所有模块安装正确并且有OpenCart文件的权限。
...
所以如果你有什么想卖的你可以自由做它（我的意思是自由,想到你会花在获得物理存储成本,然后比较其与虚拟商店建立成本。你会觉得它的自由）。
