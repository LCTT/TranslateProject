...
build-essential 包实际上是属于 Debian 的。它本身并不是一个软件。它包含了创建一个 Debian 包（deb）所需的软件包列表。这些软件包包括 libc、gcc、g++、make、dpkg-dev 等。build-essential 包包含这些所需的软件包作为依赖，所以当你安装 build-essential 时，你只需一个命令就能安装所有这些软件包。
