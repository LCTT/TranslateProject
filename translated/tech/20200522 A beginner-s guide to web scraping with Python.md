...
搜索 **href** 不会有任何结果，该死。搜索失败可能是因为 **href** 嵌套在 **itemlist** 类中。对于下一次尝试，请基于 **item_list** 搜索 。由于 “class”是 Python 中的保留字，使用 **class_**作为替代。`bs4` 函数 **soup.find_all()** 原来是 `bs4` 函数的瑞士军刀。
...
有趣的是，我发现搜索特定的类通常是一种可行的方法。通过找出对象的类型和长度，我们可以了解更多有关对象的信息。
...
可以使用 **.contents** 从 BeautifulSoup “ResultSet” 中提取内容。创建单个代表性示例这么做再恰当不过了。
...
使用 **.attr** 查找该对象内容中存在的属性。注意：**.contents** 通常会返回包含一个项目的完整列表，因此第一步是使用方括号符号为该项目建立索引。
...
所有的探索指明了方向。这就是我们上面要弄清楚的逻辑的清晰思路。
...
输出是在爱达荷州要抓取的 Family Dollar 商店的 URL 的列表。

也就是说，我仍然没有地址信息！现在，需要抓取每个城市的 URL 来得到此信息。因此，我们使用一个单一的具有代表性的示例重新启动该过程。
...
地址信息嵌套在 **type= "application/ld+json"** 中。经过大量的地理位置抓取之后，我开始认识到这是用于存储地址信息的一般结构。幸运的是，**soup.find_all()** 还可以基于 **type** 搜索 。
...
我使用 **.contents** 提取（从第二列表项）的内容（这是过滤后的默认操作）。同样，由于内容的输出是一个列表，因此我为该列表项建立了索引：
...
喔，看起来不错。此处提供的格式与 JSON 格式一致（而且，该类型的名称中确实包含 “**json**” ）。 JSON 对象可以被看做是带有嵌套字典的字典。一旦熟悉利用它来工作，实际上是非常不错的格式（当然，它比一长串 RegEx 命令更容易编程）。尽管从结构上看起来像一个 JSON 对象，但它仍然是 `bs4` 对象，需要通过正式的编程转换为 JSON 才能作为 JSON 对象进行访问：
...
在该内容中，有一个称为 **address** 的键，该键在较小的嵌套字典中具有所需的地址信息。可以这样检索：
...
好吧，这次我们很认真。现在，我可以遍历存储爱达荷州 URL 的列表：
...
我们在字典中有大量数据，还有一些额外的会使重用的数据变得比需要的更为复杂的杂项。接下来确定最终的数据组织步骤，将其转换为 **pandas** 数据框架格式，删除不需要的列 “**@type**” 和“**country**” ），并检查前五行以确保一切正常。
...
我们做到了！所有爱达荷州的 Family Dollar 商店都有一个用逗号分隔的列表。太棒了。
### 关于 Selenium 和数据抓取的说明

[Selenium][5] 是用于与网页自动交互的常用应用。为了解释为什么有时必须使用它，让我们来看一个使用 Walgreens 网站的示例。**Inspect Element** 提供了在浏览器中显示内容的代码：

![Walgreens 位置页面和代码][20]
 
**View Page Source** 提供了为 **requests** 将获得内容的代码：

![Walgreens 源代码][21]

如果这两个不一致，则有一些插件可以修改源代码，因此，可以在将页面加载到浏览器后对其进行访问。 **requests** 不能做到这一点，但是 **Selenium** 可以做到。
...
对于 Family Dollar，我不需要 Selenium，但是当呈现的内容与源代码不同时，我确实会保留它（Selenium）。
### 小结
...
\--
_作者注解：本文是2020年2月9日在俄勒冈州波特兰的[我在 PyCascades 的演讲][24]的改编。_
