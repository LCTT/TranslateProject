...
Helix：为有经验的 Linux 用户提供的基于终端的文本编辑器
======

说到[基于终端的文本编辑器][1]，通常是 Vim、Emacs 和 Nano 受到了关注。

这并不意味着没有其他这样的文本编辑器。[Neovim][2]，是 Vim 的现代增强版，是许多这样的例子之一。
...
[Helix][4] 是用 Rust 编写的，使用 Tree-sitter 进行语法高亮。开发者声称，它比 regex 高亮更快，因为 Tree-sitter 像编译器一样将代码解析成语法树，从而给出更多的代码结构信息。

你可以跟踪局部变量，计算缩进和操作选择来选择语法节点。它足够强大，即使有语法错误也能产生结果。

Helix 的主要重点是“多重选择”。这是以 [Kakoune][5] 为基础的。
