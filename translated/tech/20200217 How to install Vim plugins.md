...
每次要安装插件时，都必须在 **plug＃begin()** 和 **plug＃end()** 之间输入插件的名称和位置（上面以 NERDTree 文件管理器为例。）。如果你所需的插件未托管在 GitHub 上，你可以提供完整的URL，而不仅仅是 GitHub 用户名和项目 ID 。你甚至可以在 **~/.vim** 目录之外“安装”本地插件。
