...
2. 数组是值类型。看下面这个示例：
    ...
    语句 `b := a` 定义了一个类型是 `[5]int` 的新变量 `b`，然后把 `a` 中的内容 _复制_ 到 `b` 中。改变 `b` 对 `a` 中的内容没有影响，因为 `a` 和 `b` 是相互独立的值。^注1

### 切片

Go 语言的切片和数组的主要有如下两个区别：

1. 切片没有一个固定的长度。切片的长度不是它类型定义的一部分，而是由切片内部自己维护的。我们可以使用内置的 `len` 函数知道它的长度。^注2
2. 将一个切片赋值给另一个切片时 _不会_ 对切片内容进行复制操作。这是因为切片没有直接持有其内部数据，而是保留了一个指向 _底层数组_ ^注3 的指针。数据都保留在底层数组里。
...
Go 的切片是作为值传递而不是指针这一点不太寻常。当你在 Go 内定义一个结构体时，90% 的时间里传递的都是这个结构体的指针^注5 。切片的传递方式真的很不寻常，我能想到的唯一与之相同的例子只有 `time.Time`。
...
你可以注意到在每一个 `level` 内 `s` 的值没有被别的 `f` 的调用影响，尽管当计算更高的 `level` 时作为 `append` 的副产品，调用栈内的四个 `f` 函数创建了四个底层数组^注6 ，但是没有影响到当前各自的切片。
...
- 注1：这不是数组才有的特性，在 Go 语言里中 _一切_ 赋值都是复制过去的。
- 注2：你也可以在对数组使用 `len` 函数，但是其结果本来就人尽皆知。
- 注3：有时也叫做<ruby>后台数组<rt>backing array</rt></ruby>，以及更不严谨的说法是后台切片。
- 注4：Go 语言里我们倾向于说值类型以及指针类型，因为 C++ 的<ruby>引用<rt>reference</rt></ruby>类型这个词产生误会。但在这里我认为调用数组作为引用类型是没有问题的。
- 注5：如果你的结构体有[定义在其上的方法或者用于满足某个接口][7]，那么你传入结构体指针的比率可以飙升到接近 100%。
-注 6：证明留做习题。
