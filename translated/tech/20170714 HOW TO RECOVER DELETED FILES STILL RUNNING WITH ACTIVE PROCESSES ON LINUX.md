...
许多情况下，删除的文件都可以恢复，即使该文件具有活动进程，并且目前被单个或多个用户使用。在 Linux 系统中，每个当前正在运行的进程都会获得 ID，并将其显示为进程标识符 “PID”，并将它们存放在 /proc 目录中。这正是我们需要恢复仍在运行的已删除的文件，并且它具有PID。所以，这就是我们如何做到这一点的。
