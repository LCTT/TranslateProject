...
- 从文件或者标准输入中把文本读出来
- 提供给其它程序使用的共享库版本
- 为 Windows 提供SAPI5版本，在screen-readers或者其它支持Windows SAPI5接口程序的支持下，eSpeak仍然能正常使用
- 可移植到其它平台，包括安卓，OSX等
- 提供多种声音特性选择
...
eSpeak 用法如下：输入 espeak 按 enter 键运行程序。输入字符按 enter 转换为语音输出（译补）。使用 Ctrl+C 来关闭运行中的程序。
