...
用户会发现GParted不仅仅是唯一的工具。同样可以使用一个名叫GParted Live的操作系统，它无需安装即可直接使用。它可以在任何带有光驱或者USB接口的系统上直接运行，包括Mac操作系统。
...
此外，GParted也接受了UEFI安全启动的支持，能够在任何具有此功能的PC电脑上流畅运行。
...
使用其它操作系统的用户，比如windows或者Mac，需要支付高额的费用来使用此类软件。而Linux用户可以免费获得，而且开发者在不断的维护这个程序。
