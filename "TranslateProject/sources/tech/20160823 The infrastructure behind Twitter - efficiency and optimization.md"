...
直到2012年为止，软件团队在 Twitter 开通一个新服务还需要自己操心硬件：提出硬件的规格需求，研究机架尺寸，开发部署脚本以及处理硬件故障。同时，系统中没有所谓的“服务发现”机制，当一个服务需要调用一个另一个服务时候，需要读取一个 YAML 配置文件，这个配置文件中有目标服务对应的主机 IP 和端口信息（端口信息是由一个公共 wiki 页面维护的）。随着硬件的替换和更新，YAML 配置文件里的内容也会不断的编辑更新。在各个系统的 cache 层更新配置意味着需要花几个小时甚至几天才重新部署，所以我们只能尽量一次增加多个主机并且按次序分别部署。部署过程中的 cache 不一致导致的问题经常发生，因为有的主机在使用就的配置有的主机在用新的。有时候某一台主机在用旧的代码（例如在部署代码的时候突然下线了）会导致整个站点都行为异常。 
...
在2012/2013年的时候，Twitter 开始尝试两个新事物：服务发现（提供 ZooKeeper 集群和 Finagle 核心模块中的一个库）和 Mesos（包括在 Mesos 之上部署的一个自研的计划任务框架 Aurora ，它现在也是 Apache 基金会的一个项目）。
...
服务发现功能不再需要维护一个静态 YAML 主机列表。一个服务或者在启动后就自动注册，或者自动被 mesos 接入到一个“服务集”（就是一个 ZooKeeper 中的 znode 列表，包含角色、环境和服务名信息）。任何想要访问这个服务的组件都只需要监控这个目录就可以实时获取到一个正在工作的服务列表。
...
通过 Mesos/Aurora ，而不是使用脚本（我们曾经是 Capistrano 的重度用户）在获取一个主机列表，来分发代码并规划重启任务，服务提供者只需要将软件包上传到一个叫“包装工”的服务（给予 HDFS 的一个服务程序），再上传一个 Aurora 配置用来描述这个服务（需要多少 CPU ，多少内存，多少个实例，运行一个实例需要执行的命令等），然后 Aurora 就会完成整个部署过程。 Aurora 在现有的主机上规划实例，从“包装工”上下载代码，注册到“服务发现”，然后启动这个服务。如果整个过程中遇到失败（硬件挂了、网络中断等等）， Mesos/Aurora 会自动重新找一个主机并将服务部署上去。
...
Mesos/Aurora和服务发现这两个合在一起是一个革命性的变化。虽然接下来几年里，我们碰到了无数bug，伤透了无数脑筋，并且从分布式系统里学到了很多的教训，但是这套系统的基础是非常棒的。以前我们的团队一直在处理和思考搭配和管理硬件。而现在，工程师们只需要考虑如何优化业务以及他需要多少系统能力就可以了。同时，我们也从根本上解决了CPU利用率低的问题，因为以前服务直接安装在服务器上这种方式无法保证总能很好的利用资源，任务管理能力也很差。Mesos允许我们把多个服务打包成一个服务包，在这个包里增加一个新服务只需要增加一些硬件配额，改一行配置然后部署就可以了。
